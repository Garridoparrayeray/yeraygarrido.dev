...
const MagneticButton = ({ children, className = '' }: { children: React.ReactElement, className?: string }) => {
  ...
  return <div ref={ref} className={`flex-1 flex w-full ${className}`}>{children}</div>;
};
    ...
    <section className="pt-24 md:pt-32 pb-8 md:pb-12 px-6 md:px-12 min-h-[100svh] flex flex-col justify-between relative z-10 bg-[#f2f2f2] text-black overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto w-full text-center flex-1 flex flex-col justify-center">
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-center mt-8 md:mt-12 max-w-2xl mx-auto w-full">
          <MagneticButton>
            <a href="mailto:garridoparrayeraytx@gmail.com" className="flex-1 flex group w-full">
          ...
          <MagneticButton>
            <a href="https://linkedin.com/in/yeray-garrido" target="_blank" rel="noreferrer" className="flex-1 flex group w-full">
              <div className="bg-black border border-black w-14 h-14 md:w-16 md:h-16 flex items-center justify-center shrink-0 group-hover:bg-white transition-colors duration-300">
