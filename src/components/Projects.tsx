...
export default function Projects() {
  ...
  const uiTexts = {
    ...
    tabPortfolio: { es: "Proyectos Destacados", en: "Featured Projects", eu: "Proiektu Nabarmenak" },
    tabGithub: { es: "Repositorios GitHub", en: "GitHub Repositories", eu: "GitHub Biltegiak" },
  };

  // 1. Proyectos TOP (Se muestran en el carrusel horizontal)
  ...
  // 2. Proyectos Secundarios (Se leen del JSON y se unen para el Grid de la Modal)
  const otherProjects = otherProjectsData as FeaturedProject[];
  const allManualProjects = [...projects, ...otherProjects];
    ...
    <>
      ...
      >
        <div className="pt-16 md:pt-24 px-8 md:px-24 shrink-0 flex flex-col md:flex-row justify-between items-center md:items-end gap-6">
          ...
          <div className="hidden md:block">
            <button
              ...
              className="font-sans text-xs tracking-widest uppercase border border-white/30 px-6 py-3 hover:bg-white hover:text-black transition-colors duration-300 rounded-full cursor-pointer"
            >
              {t("projects.viewAll")}
            </button>
          </div>
                  ...
                  <div className="aspect-video bg-[#050505] mb-6 overflow-hidden relative border border-white/10 group-hover:border-white/40 transition-colors duration-500 rounded-lg">
                    {p.imageUrl ? (
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-6">
                        <span className="font-display text-[15vw] md:text-[8vw] text-white/4 uppercase">
                          0{i + 1}
                        </span>
                      </div>
                    )}
        ...
        <div className="md:hidden px-8 mt-12">
          <button
            onClick={openProjectsModal}
            className="font-sans text-xs tracking-widest uppercase border border-white/30 px-6 py-4 hover:bg-white hover:text-black transition-colors w-full text-center rounded-full cursor-pointer"
          >
            {t("projects.viewAll")}
          </button>
        </div>
      ...
      {/* MODAL PRINCIPAL (TABS: DESTACADOS Y GITHUB) */}
      {isModalOpen && (
        <div className="fixed inset-0 z-100 bg-black/95 backdrop-blur-md flex flex-col animate-modal">
          
          {/* HEADER DEL MODAL CON TABS */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-6 md:p-12 border-b border-white/10 shrink-0 gap-6">
            <div className="flex gap-6 border-b border-white/10 w-full md:w-auto">
              <button
                onClick={() => setModalTab('portfolio')}
                className={`pb-3 font-wide text-sm md:text-lg uppercase tracking-wider transition-all relative cursor-pointer ${
            ...
            <button
              ...
              className="cursor-pointer w-10 h-10 md:w-12 md:h-12 rounded-full border border-white/30 flex items-center justify-center text-white hover:bg-white hover:text-black transition-all duration-300 self-end md:self-auto"
            >
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-12 modal-scroll overscroll-contain" data-lenis-prevent="true">
            ...
            {/* VISTA 1: TODOS LOS PROYECTOS MANUALES (Projects + JSON) */}
            {modalTab === 'portfolio' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto pb-12">
                {allManualProjects.map((p) => (
                  <div
                    key={p.id}
                    onClick={() => setActiveProject(p)}
                    className="bg-[#050505] border border-white/10 hover:border-white/40 group flex flex-col h-full rounded-sm cursor-pointer overflow-hidden transition-all duration-300"
                  >
                    <div className="h-48 w-full bg-black relative overflow-hidden border-b border-white/10">
                      {p.imageUrl ? (
                        <img src={p.imageUrl} alt={l(p.title)} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-105 transition-all duration-500" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center p-4">
                          <h4 className="font-display text-2xl text-white/10 uppercase text-center wrap-break-word">
                            {l(p.title)}
                          </h4>
                        </div>
                      )}
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <h3 className="font-wide text-lg text-white mb-2 uppercase">{l(p.title)}</h3>
                      <p className="font-sans text-xs text-white/50 mb-6 flex-1 line-clamp-3">
                        {l(p.desc)}
                      </p>
                      <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-white/10">
                        {p.tech.split(", ").slice(0, 3).map((tag, j) => (
                          <span key={j} className="font-sans text-[9px] tracking-widest text-white/70 bg-white/5 border border-white/10 px-2 py-1 uppercase rounded-sm">
                            {tag}
                          </span>
                        ))}
                        {p.tech.split(", ").length > 3 && (
                          <span className="font-sans text-[9px] tracking-widest text-white/40 px-2 py-1">+{p.tech.split(", ").length - 3}</span>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            {modalTab === 'github' && (
              <>
                {loading ? (
                  ...
                  <div className="text-center text-red-400 font-sans mt-10">{error}</div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto pb-12">
                    {repos.map((repo) => (
                      <a
      ...
      {/* DETALLE PROYECTO (z-200, se abre POR ENCIMA del modal de lista) */}
      {activeProject && (
          ...
          <div className="flex-1 overflow-y-auto w-full pb-24 modal-scroll overscroll-contain" data-lenis-prevent="true">
            <div className="w-full h-[30vh] md:h-[55vh] bg-[#050505] border-b border-white/10 flex items-center justify-center relative overflow-hidden">
              {activeProject.imageUrl ? (
                ...
                <div className="font-display text-4xl md:text-8xl uppercase text-white/5 text-center p-8 wrap-break-word leading-tight">
                  {l(activeProject.title)}
                </div>
              )}
            </div>
              ...
              <div className="lg:col-span-8 flex flex-col gap-10 md:gap-16">
                <div className="space-y-5">
                  <div className="flex flex-wrap items-center gap-3">
                    <h1 className="font-wide text-2xl md:text-5xl uppercase text-white leading-tight wrap-break-word">
                      {l(activeProject.title)}
                    </h1>
              ...
              <div className="lg:col-span-4 flex flex-col gap-10 lg:pl-12 lg:border-l border-white/10 pt-10 border-t lg:pt-0 lg:border-t-0">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeProject.tech.split(", ").map((tag, j) => (
                      <span key={j} className="font-sans text-[10px] tracking-widest text-white/70 bg-white/5 border border-white/10 px-3 py-1.5 uppercase rounded-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-3 pt-4">
                  {activeProject.demoUrl && (
                    <a href={activeProject.demoUrl} target="_blank" rel="noreferrer" className="w-full text-center py-4 bg-white text-black hover:bg-white/80 transition-all font-sans text-[10px] uppercase tracking-widest font-bold rounded-sm">
                      {l(uiTexts.live)}
                    </a>
                  )}
                  {activeProject.link && (
                    <a href={activeProject.link} target="_blank" rel="noreferrer" className="w-full flex items-center justify-center gap-3 py-4 border border-white/20 hover:bg-white/10 transition-all font-sans text-[10px] uppercase tracking-widest font-bold text-white rounded-sm">
