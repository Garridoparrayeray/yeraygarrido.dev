...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(letters,
      ...
      { y: '0%', rotate: 0, opacity: 1, duration: 1.2, stagger: 0.03, ease: "power4.out", delay: 0.2 }
    );
    ...
    <section ref={containerRef} className="min-h-[100svh] flex flex-col justify-between px-4 md:px-12 pt-24 pb-8 relative z-10 origin-top">
      ...
      <h1 ref={titleRef} className="font-wide text-[12vw] md:text-[8vw] font-bold leading-[1] text-white tracking-tighter select-none flex flex-col md:flex-row justify-center items-center py-4 md:gap-[3vw]">
        ...
        <div ref={textWrapperRef} className="mt-6 flex flex-col items-center">
          ...
          <p className="mt-4 font-sans text-[10px] md:text-xs tracking-[0.2em] text-white/40 uppercase">
            {t('hero.portfolio', { year: new Date().getFullYear() })}
          </p>
        </div>
      </div>
      ...
      <div className="mt-auto pt-8 pb-8 flex flex-col items-center w-full max-w-md mx-auto">
        <div ref={buttonsRef} className="flex flex-col w-full gap-3">
          ...
          {/* AQUI ESTÁ LA MAGIA: Ruta de descarga dinámica */}
          <a href={`/CV_Yeray_Garrido_${language}.pdf`} download={`CV_Yeray_Garrido_${language.toUpperCase()}.pdf`} className="flex w-full group">
          ...
          <a href="https://github.com/Garridoparrayeray" target="_blank" rel="noreferrer" className="flex w-full group">
        ...
        <div className="mt-12 animate-bounce opacity-50">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-white"><line x1="12" y1="5" x2="12" y2="19"></line><polyline points="19 12 12 19 5 12"></polyline></svg>
        </div>
      </div>
    </section>
  );
